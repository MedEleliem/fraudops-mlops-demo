...
def __varcharProcessing__(X, varchar_process = "dummy_dropfirst"):
    ...
    if varchar_process == "drop":   
        ...
        print("Character Variables (Dropped):", object_columns)
...
def __forwardSelectionRaw__(X, y, model_type ="linear",elimination_criteria = "aic", sl=0.05):
    ...
    for i in range(X.shape[1]):
        ...
        for j in other_cols:
            ...
            pvals = pd.concat(
                [pvals, pd.DataFrame([[j, model.pvalues[j]]], columns=["Cols", "Pval"])],
                ignore_index=True,
            )
